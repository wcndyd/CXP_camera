...
void IKAPC_CC cbOnEndOfFrame(uint32_t eventType, void* pContext)
{
    ...
    if (bufferInfo.State == ITKBUFFER_VAL_STATE_FULL || bufferInfo.State == ITKBUFFER_VAL_STATE_UNCOMPLETED) {
        ...
        /// \~chinese 对于C32和AC32像素格式可以调用3DSplit接口将图像分通道和分层排布
        ...
        {
            unsigned short acquisitionLineCount = 1000;  //该参数需要提前从相机读到
            {
                //直接调用ItkBufferSave接口可以将3D图像分通道分别保存为tif图并提供了分层重排布选项:ITKBUFFER_VAL_3D_SPLIT_INTO_BLOCK_AND_STRATIFIED 或 ITKBUFFER_VAL_3D_SPLIT_INTO_BLOCK
            ...
            // 在内存中分通道重新排布并提供了分层选项
            {
                ...
                // ItkBuffer3DSplit在options1参数为ITKBUFFER_VAL_3D_SPLIT_INTO_BLOCK_AND_STRATIFIED的情况下options2参数应该传入batchCount，可以从相机AcquisitionLineCount特征获得
                ...
                if (bufferInfo.PixelFormat == ITKBUFFER_VAL_FORMAT_CALIBRATED_COORD3D_C32)
                {
                    short* C_Addr = (short*)(splitBufferInfo.BaseAddress);  // C通道short类型首地址 长度为bufferInfo.ImageWidth * bufferInfo.ImageHeight
                    uint8_t* R_Addr= (uint8_t*)(C_Addr+ bufferInfo.ImageWidth * bufferInfo.ImageHeight);    // R通道uint8_t类型首地址 长度为bufferInfo.ImageWidth * bufferInfo.ImageHeight
                }
                if (bufferInfo.PixelFormat == ITKBUFFER_VAL_FORMAT_CALIBRATED_COORD3D_AC32)
                {
                    short* A_Addr = (short*)(splitBufferInfo.BaseAddress);  // A通道short类型首地址 长度为bufferInfo.ImageWidth * bufferInfo.ImageHeight
                    short* C_Addr = (short*)(A_Addr + bufferInfo.ImageWidth * bufferInfo.ImageHeight);  // C通道short类型首地址 长度为bufferInfo.ImageWidth * bufferInfo.ImageHeight
                    uint8_t* RAddr = (uint8_t*)(C_Addr + bufferInfo.ImageWidth * bufferInfo.ImageHeight);   // R通道uint8_t类型首地址 长度为bufferInfo.ImageWidth * bufferInfo.ImageHeight
                }
            }
